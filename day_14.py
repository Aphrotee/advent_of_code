...
# The robots are good at navigating over/under each other (due to a combination of springs, extendable legs, and quadcopters), so they can share
# the same tile and don't interact with each other. Visually, the number of robots on each tile in this example looks like this:
...
# .........1.
# .......1...
# These robots have a unique feature for maximum bathroom security: they can teleport. When a robot would run into an edge of the space they're in,
# they instead teleport to the other side, effectively wrapping around the edges. Here is what robot p=2,4 v=2,-3 does for the first few seconds:
...
# ...12......
# .1....1....
# To determine the safest area, count the number of robots in each quadrant after 100 seconds. Robots that are exactly in the
# middle (horizontally or vertically) don't count as being in any quadrant, so the only relevant robots are:
...
# Your puzzle answer was 230900224.
...
# If they're the same type of robots, they should have a hard-coded Easter egg: very rarely, most of the robots should arrange
...
def get_new_position(x, y, vx, vy, seconds):
    ...
    if nx >= 0:
        ...
        nx = (cols - (abs(nx) % cols)) % cols
    
    if ny >= 0:
        ny %= rows
    else:
        ny = (rows - (abs(ny) % rows)) % rows
...
def get_quadrant(x, y):
    ...
    if 0 <= x < midx and 0 <= y < midy:
        return 1
    elif midx < x < cols and 0 <= y < midy:
        return 2
    elif 0 <= x < midx and midy < y < rows:
        return 3
    elif midx < x < cols and midy < y < rows:
        return 4   
    return 0 

def check_for_tree(bathroom, rows, cols):
    count = 0
...
def main():
    ...
    for second in range(1000, 10000):
        bathroom = [["." for _ in range(101)] for _ in range(103)]
        for robot in robots:
            x, y = get_new_position(robot["x"], robot["y"], robot["vx"], robot["vy"], second)
            bathroom[y][x] = "X"
        if check_for_tree(bathroom, 103, 101):
            print(second)
            return
        
    # for robot in robots:
    #     x, y = get_new_position(robot["x"], robot["y"], robot["vx"], robot["vy"])
    #     print(x, y)
