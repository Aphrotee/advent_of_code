...
# the equations that could possibly be true. In the above example, the sum of the test values for the three equations listed above is 3749.

# Determine which equations could possibly be true. What is their total calibration result?

# Your puzzle answer was 932137732557.
...
# Your puzzle answer was 661823605105500.
...
def check_combination(ans, params):
    ...
    def backtrack(index, value):
        ...
        return backtrack(index + 1, value + params[index]) or \
            backtrack(index + 1, value * params[index]) or \
            backtrack(index + 1, (value * get_order(params[index])) + params[index])
    
    return backtrack(1, params[0])
...
while True:
    ...
    line = line.split(":")
    ans = int(line[0])
    params = list(map(int, line[1].strip().split()))
    if check_combination(ans, params):
        answer += ans
print(answer)